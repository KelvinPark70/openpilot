...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):
    ...
    if candidate == CAR.SANTAFE:
      ...
      ret.lateralTuning.init('indi')
      ret.lateralTuning.indi.innerLoopGain = 3.6 #3.0
      ret.lateralTuning.indi.outerLoopGain = 2.6 #2.0
      ret.lateralTuning.indi.timeConstant = 2.0
      ret.lateralTuning.indi.actuatorEffectiveness = 2.0 #1.5
      ret.steerActuatorDelay = 0.08 # Stinger GT Limited AWD 3.3T stock value (Tunder's 2020) 
      ret.steerLimitTimer = 0.04 # stock is 0.01 but 0.04 seems to work well
